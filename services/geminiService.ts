...
export const generateAnalysis = async (projectName: string, prompt: string, faction: Faction, fileParts: Part[] | null): Promise<AnalysisResult> => {
    ...
    **Faction Bias:**
    ...
    const response = await ai.models.generateContent({
        model: 'gemini-3-pro-preview', // Upgraded to 2.5 Pro for complex reasoning
        contents: { parts },
        config: {
            systemInstruction,
...
const generateImageWithFallback = async (prompt: string, aspectRatio: string): Promise<string> => {
    ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });
    
    const modelsToTry = ['imagen-4.0-generate-001', 'imagen-3.0-generate-001'];
    ...
    throw new Error("Image generation failed after trying available models. Please try again later or refine your prompt.");
};
...
export const generateTechnicalDrawingImage = async (analysisResult: AnalysisResult, specificPrompt: string, fileUrls?: string[]): Promise<string> => {
    ...
    // Check if we have reference images. If so, we MUST use a model that supports image input (Gemini).
    // If not, we should use the dedicated image generation model (Imagen) via our fallback helper.
    if (fileUrls && fileUrls.length > 0) {
        ...
        // This is an image-to-image (edit/contextual) task.
        ...
        const fullPrompt = `Using the provided image as a reference, generate a technical drawing for the following request: "${specificPrompt}". Use the other provided text context to inform the drawing. Context: ${context}`;
        ...
        throw new Error('Image generation from reference failed.');
    } else {
        // Text-to-Image task: Use Imagen with Fallback
        const fullPrompt = `A professional technical drawing blueprint of ${specificPrompt}. The drawing should be clean, 2D, white background with blue lines. Product Context: ${analysisResult.product_name}. ${context.substring(0, 400)}`; 
        return await generateImageWithFallback(fullPrompt, '16:9');
    }
};
...
export const generateInspirationalImage = async (prompt: string, aspectRatio: string): Promise<string> => {
    return await generateImageWithFallback(prompt, aspectRatio);
};

const factionPromptsSchema = {
    type: Type.OBJECT,
    properties: {
        prompts: {
            type: Type.ARRAY,
            items: { type: Type.STRING },
            description: "An array of 3-4 diverse, creative, and highly detailed prompts for a photorealistic image generator like Imagen."
        }
    },
    required: ["prompts"]
};
...
export const generateFactionInspirationalPrompts = async (result: AnalysisResult): Promise<string[]> => {
    ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });
    const systemInstruction = `You are a creative director for an advanced engineering firm specializing in cutting-edge materials and manufacturing processes. Your task is to generate 3-4 distinct, highly detailed, and creative prompts for an image generation AI (like Imagen). These prompts should visualize innovative redesigns of a given product, strictly adhering to the "Advanced Materials & Processes" philosophy.

Focus on concepts like:
- Graphene composites, translucent or semi-transparent materials revealing internal structures.
- Single-crystal alloys with flawless, exotic finishes.
- Meta-materials with impossible-looking geometric structures.
- Manufacturing techniques like atomic layer deposition or nano-imprinting, emphasizing extreme precision and detail.
- A clean, futuristic, "product shot" or "technical diagram" aesthetic.

The output must be a JSON object containing an array of prompt strings.`;

    const prompt = `
    Product Name: ${result.product_name}
    Product Summary: ${result.executive_summary}
    Key Components: ${result.designDocument.component_designs.map(c => c.component_name).join(', ')}

    Generate 3-4 diverse, creative, and highly detailed prompts for a photorealistic image generator to visualize redesigns of this product.
    `;
    ...
    const response = await ai.models.generateContent({
        ...
        config: {
            systemInstruction,
            responseMimeType: 'application/json',
            responseSchema: factionPromptsSchema,
        },
    });
...
export const extractProjectDetailsFromVideo = async (videoPart: Part): Promise<ExtractedProjectDetails> => {
    ...
    const response = await ai.models.generateContent({
        ...
        contents: { parts: [videoPart, { text: "Analyze this video for reverse engineering. Identify the product shown. Infer a project name, a detailed description of the mechanism or product, relevant tags, and create an initial prompt for a full analysis. Return a JSON object matching the schema." }] },
        config: {
            responseMimeType: 'application/json',
            responseSchema: extractedDetailsSchema,
        }
...
export const extractProjectDetailsFromVideoUrl = async (url: string): Promise<ExtractedProjectDetails> => {
    ai = new GoogleGenAI({ apiKey: process.env.API_KEY! });
    const prompt = `Analyze the video at this URL: ${url}. Determine what product or mechanism is being shown. Infer a project name, a description, relevant tags, and create an initial prompt for a reverse-engineering analysis. Search the web for details if needed. 
    
    IMPORTANT: Return ONLY a raw JSON object. Do not include any markdown formatting, explanations, or introductory text.
    ...
    JSON Structure:
    {
      ...
      "tags": ["tag1", "tag2"],
      "initialPrompt": "Initial analysis prompt"
    }
    `;
    
    // Search grounding doesn't support strict JSON response schemas in all cases with googleSearch enabled.
    // We rely on the text prompt to request JSON and parse it.
    const response = await ai.models.generateContent({
        model: 'gemini-2.5-flash',
        contents: prompt,
        config: {
    ...
    if (jsonBlockMatch) {
        ...
        // 2. Fallback: find the first '{' and last '}' to strip potentially chatty intro/outro text
...
export const identifyImageFromWeb = async (imagePart: Part): Promise<{ summary: string; sources: any[] }> => {
    ...
    const response = await ai.models.generateContent({
        model: "gemini-2.5-flash",
        contents: { parts: [imagePart, { text: "Identify the object in this image and provide a brief summary. Search the web for relevant information and sources." }] },
        config: {
            tools: [{ googleSearch: {} }],
        },
    });
