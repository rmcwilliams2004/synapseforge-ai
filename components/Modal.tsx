    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[200] p-4 animate-fade-in" onClick={onClose}>
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-xl border border-gray-200 dark:border-gray-700 flex flex-col overflow-hidden animate-scale-in" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-5 border-t border-gray-100 dark:border-gray-800 flex justify-end gap-3 bg-gray-50 dark:bg-slate-800/50">
          {cancelText !== null && (
            <button onClick={onClose} className="px-6 py-2 text-sm font-bold text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-white transition-colors focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 active:scale-95 rounded-lg">
              {cancelText ?? 'Cancel'}
            </button>
          )}
          <button 
            ...
            className="px-8 py-2 bg-brand-cyan text-white font-bold text-sm rounded-lg hover:bg-cyan-600 transition-all disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-brand-cyan focus:ring-offset-2 dark:focus:ring-offset-gray-900 active:scale-95"
          >
            {confirmText}
          </button>
        </div>
