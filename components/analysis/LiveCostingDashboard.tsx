    ...
    return (
        <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 space-y-6">
            <div className="flex justify-between items-start">
                <div>
                    <h4 className="text-xl font-semibold text-brand-light">Live Costing Dashboard</h4>
                    <p className="text-sm text-gray-400">Modify parameters and recalculate the cost estimate in real-time.</p>
                </div>
                {!isViewer && (
                     <button
                        onClick={recalculate}
            ...
            <div>
                <h5 className="font-semibold text-gray-300 mb-2">Editable Bill of Materials</h5>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left text-gray-400">
                        <thead className="text-xs text-gray-300 uppercase bg-gray-700/50">
                            <tr>
                        ...
                        <tbody>
                            {editableBom.map(item => (
                                <tr key={item.part_number} className="border-b border-gray-700">
                                    ...
                                    <td className="px-4 py-2">
                                        <input 
                                            ...
                                            className="w-20 p-1 bg-gray-900 border border-gray-600 rounded"
                                    ...
                                    <td className="px-4 py-2">
                                        <input 
                                            ...
                                            className="w-full p-1 bg-gray-900 border border-gray-600 rounded"
                                            disabled={isViewer || isRecalculating}
                                        />
                                    </td>
                                </tr>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div>
                    <h5 className="font-semibold text-gray-300 mb-2">Original Estimate</h5>
                     <div className="p-4 bg-gray-900/30 rounded-lg border border-gray-700/50">
                        <p className="text-2xl font-bold text-gray-300">{originalEstimate.total_estimate_range}</p>
                        <CostTable estimate={originalEstimate} />
                    </div>
                </div>
                 <div>
                    <h5 className="font-semibold text-gray-300 mb-2">Current Estimate</h5>
                    <div className={`p-4 rounded-lg border ${hasChanges ? 'bg-cyan-900/20 border-brand-cyan' : 'bg-gray-900/30 border-gray-700/50'}`}>
                        <p className="text-2xl font-bold text-white">{currentEstimate?.total_estimate_range}</p>
