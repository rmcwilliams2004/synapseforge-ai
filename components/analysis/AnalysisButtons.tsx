...
export const ReadAloudButton = ({ text, tts, voice }: { text: string, tts: any, voice: string }) => (
    <button onClick={() => tts.speak(text, voice)} className="p-2 text-gray-400 hover:text-brand-cyan transition focus:outline-none focus:ring-2 focus:ring-brand-cyan focus:ring-offset-2 dark:focus:ring-offset-gray-900 rounded-md active:scale-95" title="Read Aloud">
...
export const CommentButton = ({ sectionId, sectionTitle, onToggle, count, isOpen }: any) => (
    <button onClick={() => onToggle(sectionId, sectionTitle)} className={`flex items-center gap-1.5 p-2 rounded-md transition-colors focus:outline-none focus:ring-2 focus:ring-brand-cyan focus:ring-offset-2 dark:focus:ring-offset-gray-900 active:scale-95 ${isOpen ? 'bg-brand-cyan/20 text-brand-cyan' : 'text-gray-400 hover:text-gray-600'}`}>
    ...
    <div className="flex gap-4">
      ...
      >
        <button 
          ...
          className="px-6 py-3 bg-cyan-600 hover:bg-cyan-500 rounded-full font-black text-[10px] tracking-widest uppercase flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-cyan-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 active:scale-95"
        >
      ...
      >
        <button 
          ...
          className="px-6 py-3 bg-amber-600 hover:bg-amber-500 rounded-full font-black text-[10px] tracking-widest uppercase flex items-center gap-2 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 dark:focus:ring-offset-gray-900 active:scale-95"
        >
