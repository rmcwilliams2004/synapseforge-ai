...
const SimulationTypeButton = ({ type, currentType, setType }: { type: SimulationType, currentType: SimulationType, setType: (type: SimulationType) => void }) => {
    ...
    return (
        <button onClick={() => setType(type)} className={`py-2 px-4 rounded-lg font-semibold transition-colors text-sm ${isActive ? 'bg-purple-600 text-white' : 'bg-gray-700 hover:bg-gray-600'}`}>
        ...
        <div className="mb-6 animate-fade-in" style={{ animationDelay: '150ms' }}>
            ...
            <div className="pl-8">
                 <div className="bg-gray-800/50 p-6 rounded-lg border border-gray-700 space-y-6">
                    <div>
                        <h4 className="text-lg font-semibold text-brand-light mb-3">1. Select Simulation Type</h4>
                     ...
                     <div>
                        <h4 className="text-lg font-semibold text-brand-light mb-3">2. Select Component(s) for Analysis</h4>
                        <div className="max-h-48 overflow-y-auto space-y-2 p-3 bg-gray-900/50 border border-gray-600 rounded-md">
                            {bom.map(item => (
                                <label key={item.part_number} className="flex items-center gap-3 p-2 rounded hover:bg-gray-700/50 transition cursor-pointer">
                                    <input
                                        type="checkbox"
                                        checked={selectedComponents.has(item.name)}
                                        onChange={() => handleToggleComponent(item.name)}
                                        className="h-4 w-4 rounded border-gray-500 text-purple-500 focus:ring-purple-500 bg-gray-700"
                                    />
                                    <span className="text-sm text-gray-300">{item.name} <span className="text-gray-500">({item.material})</span></span>
                                </label>
                            ))}
                        </div>
                    </div>
                    ...
                    {simulationResult && (
                         <div className="pt-6 border-t border-gray-700">
                             <h4 className="text-xl font-semibold text-brand-light mb-4">Simulation Results: {simulationResult.type} on {simulationResult.componentName}</h4>
                             {simulationResult.isLoading && (
                                <div className="p-4 bg-gray-900/50 rounded-lg text-center">
                                    ...
                                    <p className="text-purple-300 font-semibold">Running Simulation...</p>
                                    <p className="text-sm text-gray-400 font-mono mt-2 h-6">{status[status.length - 1]}</p>
                                </div>
                             )}
                             {simulationResult.error && (
                                 <p className="text-red-400 p-4 bg-red-900/30 rounded-lg">{simulationResult.error}</p>
                             )}
                             {!simulationResult.isLoading && !simulationResult.error && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                                    <div className="space-y-4">
                                        <div>
                                            <h5 className="font-semibold text-gray-300 mb-1">Summary</h5>
                                            <p className="text-sm text-gray-400 bg-gray-900/30 p-3 rounded">{simulationResult.summary}</p>
                                        </div>
                                         <div>
                                            <h5 className="font-semibold text-gray-300 mb-1">Key Findings</h5>
                                    ...
                                    <div>
                                        {simulationResult.imageUrl ? (
                                             <img src={simulationResult.imageUrl} alt={`Simulation result for ${simulationResult.componentName}`} className="w-full rounded-lg border-2 border-gray-600" />
                                        ) : (
                                            <div className="aspect-video bg-gray-700 rounded-lg flex items-center justify-center text-gray-500">Visualization not available</div>
                                        )}
                                    </div>
                                </div>
                             )}
