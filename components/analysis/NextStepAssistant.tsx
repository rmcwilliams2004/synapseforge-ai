...
const SuggestionCard: React.FC<{ suggestion: NextStepSuggestion; onAction: (actionId: string) => void; }> = ({ suggestion, onAction }) => (
    <button
        ...
        className="flex items-center gap-4 text-left p-4 rounded-lg bg-white dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 hover:bg-cyan-50 dark:hover:bg-cyan-900/40 hover:border-brand-cyan transition-all duration-200 hover:-translate-y-1 w-full focus:outline-none focus:ring-2 focus:ring-brand-cyan focus:ring-offset-2 dark:focus:ring-offset-gray-900 active:scale-[0.98]"
    >
        <div className="p-3 bg-gray-100 dark:bg-gray-700 rounded-full text-brand-cyan">
            {IconMap[suggestion.icon]}
        </div>
        <div>
            <h4 className="font-bold text-gray-900 dark:text-brand-light">{suggestion.title}</h4>
    ...
    return (
        <div className="mb-6 animate-fade-in">
            <div className="flex justify-between items-center mb-3 pb-2 border-b-2 border-cyan-500/30 dark:border-cyan-800/50">
                ...
                <button onClick={onRefresh} disabled={isLoading} className="p-2 text-gray-400 rounded-md hover:bg-gray-200 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white disabled:opacity-50 transition focus:outline-none focus:ring-2 focus:ring-brand-cyan focus:ring-offset-2 dark:focus:ring-offset-gray-900 active:scale-95" title="Refresh Suggestions">
            ...
            <div className="pl-8">
                {isLoading && suggestions.length === 0 && <p className="text-gray-500 dark:text-gray-400">Thinking of next steps...</p>}
